...
public class Main {
  ...
  public static ValueMap2D getSimplexFractal(int xSize, int ySize, int seed, double[] sizes, double[] weights) {
    ...
    for (int i = 0; i < rnoises.length; i++) {
      ...
      final int fi = i;
      rnoises[i] = WorldUtils.mapOf(xSize, ySize,
          (x, y) -> (weights[fi] * n.GetNoise((float) (x * sizes[fi]), (float) (y * sizes[fi]))));
    }
  ...
  public static ValueMap2D getHeightMap(int xsize, int ysize, int seed) {
    ...
    double[] sizes = { Math.pow(2, -1), Math.pow(2, 0), Math.pow(2, 1), Math.pow(2, 2), Math.pow(2, 3), Math.pow(2, 4),
        Math.pow(2, 5) };
    double[] weights = { 17 / 70, 15 / 70, 13 / 70, 10 / 70, 7 / 70, 5 / 70, 3 / 70 };

    ValueMap2D rvals = getSimplexFractal(xsize, ysize, seed + 3, sizes, weights);
    ValueMap2D cvals = WorldUtils.mapOf(xsize, ysize,
        (x, y) -> Double.valueOf(cnoise.GetNoise(x * cscale, y * cscale)));
    ValueMap2D mvals = WorldUtils.mapOf(xsize, ysize,
        (x, y) -> Double.valueOf(Math.abs(mnoise.GetNoise(x * mscale, y * mscale))));

    return WorldUtils.apply(new ValueMap2D[] { rvals, cvals, mvals },
        xarr -> OtherUtils.clamp(0.4 * xarr[0] + 0.3 * xarr[1] + 0.3 * xarr[2], 0, 1));
  }
  ...
  public static void main(String[] args) throws InterruptedException {
     ...
     * 
     * WorldTemplate wt = new WorldTemplate(new
     * File("./Resources/Templates/2PoleContinental")); WorldScript ws = new
     * WorldScript( new JavaScript(new TextFile(wt.getMain()).getContent(),
     * System.out), canvas, 1);
     * 
     * ws.run();
     */
